...
| Phiên bản | 1.0 |
| Ngày lập | 19/09/2026 |
| Trạng thái | Thiết kế đã lưu theo plan được người dùng yêu cầu; chưa tạo/chạy migration SQL |
| Phạm vi | Database cho Web người học và Web Admin; không thêm chức năng ngoài MVP |
| Căn cứ | [SRS 0.4](../srs.md), [Kế hoạch web 1.1](../web-mvp-plan.md) |
| Môi trường | Supabase local qua CLI/Docker khi phát triển; Supabase Cloud khi deploy; cùng lịch sử migration |

Thiết kế gồm **33 bảng nghiệp vụ: 4 Identity, 16 Content và 13 Learning**, chưa tính bảng do Supabase quản lý. Mục tiêu là bảo toàn lịch sử, tránh kết quả trùng và giữ ranh giới dữ liệu giữa các service. Các mô tả constraint, transaction và kiểm thử trong tài liệu là đặc tả để triển khai, không phải tuyên bố đã có database chạy.
...
1. Khóa course chứa thay đổi, rồi các topic/lesson/assessment/question liên quan theo thứ tự ID ổn định. Các thao tác sửa draft cũng phải lấy khóa cha trước khi sửa con; tránh thêm option đúng lúc transaction khác đang xuất bản.
...
**Trạng thái toàn bộ ca dưới đây: chưa chạy.** Đây là đặc tả kiểm thử cho bước tạo migration/backend, không phải kết quả kiểm thử database. SQL tests dùng database local biệt lập; test đồng thời cần ít nhất hai kết nối thật và điều khiển điểm chờ; OAuth/DTO/Storage cần test tích hợp qua đúng API.
