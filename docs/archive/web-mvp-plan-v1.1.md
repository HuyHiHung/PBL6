...
| Căn cứ dữ liệu | [Thiết kế database 1.0](docs/database-design.md) |
...
| Trạng thái | Đã lưu plan và thiết kế database; các hạng mục triển khai ứng dụng/database chưa được thực hiện |
...
Các lệnh dưới đây là quy trình cần thiết lập tại WEB-00, chưa được chạy trong lần lưu tài liệu này. Khi CLI đã được ghim, gọi qua `pnpm exec supabase`:
...
- [ ] Kiểm tra Docker engine, cài/ghim CLI và chạy Supabase local; khai báo schema/runtime roles và kết nối đúng môi trường.
- [ ] Viết migration khởi tạo theo thiết kế, seed tối thiểu, bootstrap tài khoản quản trị thử/media; kiểm tra dựng sạch và nâng cấp trong CI cùng kiểm tra kiểu/lint/build.
- [ ] Chạy skeleton web/admin/API local có health check và log; hướng dẫn khởi động/dừng/tái tạo môi trường.
...
[Thiết kế database](docs/database-design.md) đã được lưu, quy định dữ liệu từng service, ERD, constraint/index, giao dịch, migration và ca kiểm thử. Hợp đồng API đầy đủ, cấu hình OAuth/môi trường, migration SQL thực thi, báo cáo kiểm thử và hướng dẫn deploy sẽ được tạo trong các hạng mục triển khai tương ứng. Việc lưu tài liệu không đánh dấu các hạng mục đó đã hoàn thành.
